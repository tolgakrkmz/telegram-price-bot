...
async def render_favorites_text(favorites: dict) -> str:
    ...
    for pid, p in favorites.items():
        ...
        store_name = p.get('supermarket', {}).get('name') if isinstance(p.get('supermarket'), dict) else p.get('store', 'Unknown')
    ...
    for store, products in grouped_favorites.items():
        ...
        for pid, p in products:
            ...
            current_match = next(
                (item for item in fresh_results if (item.get('supermarket', {}).get('name') == store or item.get('store') == store) and item['name'] == name), 
                None
            )
...
async def view_price_history_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    ...
    api_match = next((p for p in api_results if p['store'] == store and p['name'] == product_name), None)
    ...
    if not sorted_dates:
        await query.message.reply_text("❌ No historical data available for this product yet.")
        return
...
async def add_to_favorite_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = f"⭐ *{product['name']}* added to favorites." if added else "ℹ️ Already in favorites."
