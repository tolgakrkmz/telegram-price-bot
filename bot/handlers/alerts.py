...
from db.storage import get_favorites, get_all_favorites, update_price_history
...
async def global_price_update(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for pid, p in unique_prods.items():
        ...
        if not isinstance(new_results, list): continue
                    ...
                    if pid in user_favs:
                        try:
                            await context.bot.send_message(
                                chat_id=user_id, text=msg, parse_mode=constants.ParseMode.MARKDOWN
                            )
